...
public class RMIClient extends UnicastRemoteObject implements RMIVirtualView, StringEventListener, DoubleEventListener {
    ...
    @Override
    public void sendAvailableGames(List<Controller> games) throws RemoteException {
        view.displayAvailableGames(games);
    }
    ...
    @Override
    public void onDoubleEvent(EventType type, double v1, double v2) {
        switch (type) {
            case GAME_SELECTION:
                try {
                    server.addPlayerToGame(this, (int)v1);
                } catch (RemoteException e) {
                    System.err.println("Unable to join game.");
                }
            case GAME_CREATION:
                try {
                    server.createNewGame((int)v1, (int)v2);
                    view.updateState(false);
                }
                catch (RemoteException e) {
                    System.err.println("Unable to create new game.");
                }
        }
    }
}
